...
(3) (4.5 pts) Using the definition for writing the values of the stress tensor in terms of the strain tensor we get that 
...
(4) (7 pts) Since we have that the traction vectors both the top and bottom faces are $0$, then we have that $\left[\breve{\boldsymbol{t}}(X_1,X_2,X_3;\hat{\boldsymbol{E}}_2)\right] = \boldsymbol{0}$ and $\left[\breve{\boldsymbol{t}}(X_1,X_2,X_3;\hat{\boldsymbol{E}}_3)\right] = \boldsymbol{0}$, so 
...
(5) (3.5 pts) Then replacing $\lambda_{2} - 1$ and $\lambda_{3} - 1$ we have that 
...
(6) (2 pts) Since the integral $\int_{\Gamma(X_1)} \, d\Gamma = A(X_1)$, then we have that 

 $$\boldsymbol{F}(X_1,\hat{\boldsymbol{E}}_1) = \begin{pmatrix} \frac{A(X_1) \mu  (2 \mu  + 3 \lambda) (-1 + \lambda_1)}{\mu + \lambda} \\ 0 \\ 0 \end{pmatrix}$$

(7) (3.5 pts)  Using the definitions given for $\mu $ and $\lambda$ we have that $\frac{\mu  (2 \mu  + 3 \lambda) }{\mu + \lambda} = E$, so
...
(b) (5 pts) The maximum value of $M(X_1)$ from $0 ~{\rm m}$ to $4.5 ~{\rm m}$ is $17kNm$ at $0 ~{\rm m}$. Since its positive the value of $X_2$ that will maximize the stress is clearly $\frac{h}{2}$. $I = \frac{h^4}{12}$ for this cross section. $\sigma_{max} = \frac{M_{max}\frac{h}{2}}{I} = \frac{(17~\rm kNm)(.0125~\rm m)}{3.255 \times 10^{-8} ~\rm m^4} = 6528 ~\rm MPa$.
...
(c) (10 pts) Integrating the moment equation twice we get that  
...
(8 pts) Plugging these reaction force/moment values into our equation for the displacement we get that
